...
class distance_between_2_nodes_in_binary_trees
{
    ...
    */
    ...
    // otherwise returns -1
    int findLevel1(Node* root, int k, int level)
    {
        // Base Case
        if (root == NULL)
            return -1;
        ...
        int l = findLevel1(root->left, k, level + 1);
        return (l != -1) ? l : findLevel1(root->right, k, level + 1);
    }
    ...
    */
    ...
    int findDistance2(Node* root, int a, int b)
    {
        // Your code here
