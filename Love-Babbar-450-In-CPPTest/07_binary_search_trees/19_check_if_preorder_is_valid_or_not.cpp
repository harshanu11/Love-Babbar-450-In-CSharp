...
*/
void constructBST(nodeBT** root, int key) {
    if (!(*root)) return;

    if ((*root)->data > key) {
        if ((*root)->left == NULL) (*root)->left = new nodeBT(key);
        else constructBST(&(*root)->left, key);
    }
    else {
        if ((*root)->right == NULL) (*root)->right = new nodeBT(key);
        else constructBST(&(*root)->right, key);
    }
}

nodeBT* constructTree2(int pre[], int size)
{
    nodeBT* root = new nodeBT(pre[0]);
